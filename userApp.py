...
class DatabaseManager:
    ...
    *********************************
    ...
    def plot_number_of_listings_by_state(self):
        """
        Fetches and plots the number of listings per state.
        """
        query = "SELECT state, COUNT(*) AS number_of_listings FROM address GROUP BY state;"
        results = self.fetch_data(query)
        if results:
            states = [row[0] for row in results]
            number_of_listings = [row[1] for row in results]

            # Create a bar plot
            plt.figure(figsize=(10, 6))
            plt.bar(states, number_of_listings, color='skyblue')
            plt.xlabel('State')
    ...
    def plot_number_of_listings_by_owner(self):
        """
        Fetches and plots the number of listings per state.
        """
        query = "select Personid, sum(number_of_listing) from owner group by Personid;"
        results = self.fetch_data(query)
        if results:
            states = [row[0] for row in results]
            number_of_listings = [row[1] for row in results]
